...
export default function ManagementVirtualProvider() {
  ...
  const fetchGames = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('virtual_games').select('*').order('id', { ascending: true });
  ...
  const filteredGames = games.filter((game) => (game.category || 'VIRTUAL').toUpperCase() === activeCategory);
  ...
  const handleDelete = async (id, e) => {
    ...
    try {
      const { error } = await supabase.from('virtual_games').delete().eq('id', id);
      if (error) throw error;
      fetchGames();
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-black tracking-wide text-blue-500 uppercase">
            Management Virtual Provider
          </h1>
          <p className="text-xs text-gray-400 mt-1">
            Total: <span className="font-bold text-yellow-400">{games.length}</span> Assets terdaftar
          </p>
        </div>
              ...
              {loading ? (
                ...
                filteredGames.map((game, index) => (
                  <tr key={game.id} className={`transition ${darkMode ? 'hover:bg-blue-500/5' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="p-4">
                      <div 
                        onClick={() => router.push(`/admin/virtual/${game.name.toLowerCase()}?category=${activeCategory.toLowerCase()}`)}
          ...
          <div className={`w-full max-w-lg p-6 sm:p-8 rounded-3xl border shadow-2xl relative ${darkMode ? 'bg-[#0b0e2d] border-blue-900 text-white' : 'bg-white border-gray-200 text-gray-900'}`}>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              <div>
                ...
                <input
                  type="text"
                  placeholder="Contoh: SABA Virtual / Pragmatic Virtual"
                  value={name}
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full p-3.5 rounded-2xl border text-sm font-bold opacity-80 cursor-not-allowed ${
                    darkMode 
                      ? 'bg-[#06081d] border-blue-900/60 text-blue-400' 
