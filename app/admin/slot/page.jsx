...
export default function ManagementProvider() {
  ...
  const fetchGames = async () => {
    try {
      setLoading(true);
      const { data, error } = await supabase.from('slot_games').select('*').order('id', { ascending: true });
  ...
  const filteredGames = games.filter((game) => (game.category || 'SLOT').toUpperCase() === activeCategory);
              ...
              {loading ? (
                ...
                filteredGames.map((game, index) => (
                  <tr key={game.id} className={`transition ${darkMode ? 'hover:bg-blue-500/5' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="p-4">
                      {/* KETIKA GAMBAR DIKLIK, PINDAH KE HALAMAN DETAIL GAME */}
                      <div 
                        onClick={() => router.push(`/admin/slot/${game.name.toLowerCase()}?category=${activeCategory.toLowerCase()}`)}
