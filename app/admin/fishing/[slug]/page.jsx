...
export default function FishingProviderDetail() {
  const params = useParams();
  const searchParams = useSearchParams();
  const router = useRouter();

  const rawSlug = params?.slug || '';
  const providerName = decodeURIComponent(rawSlug).replace(/-/g, ' ').toUpperCase();
  const categoryFromQuery = searchParams.get('category') || 'FISHING';
  ...
  const fetchFishingGames = async () => {
    try {
      setLoading(true);
      // Mengambil data dari tabel (sesuaikan nama tabel database Anda)
      const { data, error } = await supabase
        .from('slot_games')
        .select('*')
        .ilike('category', categoryFromQuery)
        .order('id', { ascending: true });

      if (error) throw error;
      
      if (data) {
        const filteredByProvider = data.filter(
          (item) => (item.provider || '').toUpperCase() === providerName || item.name.toUpperCase().includes(providerName)
        );
        setGames(filteredByProvider.length > 0 ? filteredByProvider : data);
      }
    } catch (error) {
      console.error('Gagal memuat game fishing:', error.message);
    } finally {
      setLoading(false);
    }
  };

  // Pisahkan data berdasarkan tipe (POPULER & REGULER)
  ...
  const handleDelete = async (id, e) => {
    ...
    try {
      const { error } = await supabase.from('slot_games').delete().eq('id', id);
      if (error) throw error;
      fetchFishingGames();
    } catch (error) {
      ...
      if (editingId) {
        const { error } = await supabase
          ...
          .update({ 
            ...
            image_url: imageUrl, 
            type: modalType, 
            category: categoryFromQuery.toUpperCase() 
          })
          .eq('id', editingId);
        if (error) throw error;
  ...
  return (
    <div className={`min-h-screen p-4 sm:p-8 transition-colors duration-300 ${darkMode ? 'bg-[#06081d] text-white' : 'bg-gray-100 text-gray-900'}`}>
      ...
      {/* HEADER SECTION */}
      <div className={`p-6 rounded-2xl border shadow-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 ${darkMode ? 'bg-[#0b0e2d] border-cyan-950' : 'bg-white border-gray-200'}`}>
        <div>
          <p className="text-xs font-bold tracking-widest text-gray-400 uppercase mb-1">MANAJEMEN GAME</p>
          <h1 className="text-2xl sm:text-3xl font-black tracking-wider text-cyan-400 uppercase">
            {providerName} <span className={darkMode ? 'text-white' : 'text-gray-800'}>GAMES</span>
          </h1>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            className="bg-cyan-600 hover:bg-cyan-500 text-white font-extrabold text-xs sm:text-sm px-5 py-2.5 rounded-xl shadow-lg shadow-cyan-600/30 transition flex items-center gap-2"
          >
            + TAMBAH REGULER
          </button>
        </div>
      ...
      <div>
        ...
        <div className={`rounded-2xl border overflow-hidden shadow-2xl ${darkMode ? 'bg-[#0b0e2d] border-cyan-950' : 'bg-white border-gray-200'}`}>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className={`text-xs uppercase font-black tracking-wider ${darkMode ? 'bg-[#080b22] text-gray-300 border-b border-cyan-950' : 'bg-gray-800 text-white'}`}>
                  <th className="p-4 w-16 text-center">NO</th>
                  <th className="p-4">NAMA</th>
                  <th className="p-4 w-40">GAMBAR</th>
                  <th className="p-4 text-center w-28">AKSI</th>
                ...
                {loading ? (
                  ...
                  regularGames.map((game, index) => (
                    <tr key={game.id} className={`transition ${darkMode ? 'hover:bg-cyan-500/5' : 'hover:bg-gray-50'}`}>
                      ...
                      <td className="p-4">
                        <div className="w-28 h-12 rounded-lg overflow-hidden border border-cyan-400/30 bg-black/50 shadow-inner flex items-center justify-center">
                          <img src={game.image_url} alt={game.name} className="w-full h-full object-cover" />
                        </div>
                      </td>
                      <td className="p-4 text-center">
          ...
          <div className={`w-full max-w-lg p-6 sm:p-8 rounded-3xl border shadow-2xl relative ${darkMode ? 'bg-[#0b0e2d] border-cyan-950 text-white' : 'bg-white border-gray-200 text-gray-900'}`}>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              <div>
                ...
                <input
                  type="text"
                  placeholder="Contoh: Oneshot Fishing"
                  value={gameName}
              ...
              <div>
                ...
                <select
                  ...
                  className={`w-full p-3.5 rounded-2xl border text-sm font-bold outline-none transition shadow-inner ${
                    darkMode 
                      ? 'bg-[#06081d] border-cyan-950 text-white focus:border-cyan-400' 
                      : 'bg-gray-50 border-gray-200 text-black focus:border-cyan-500'
                  }`}
                >
