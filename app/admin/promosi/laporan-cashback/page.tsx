...
export default function LaporanCashbackPage() {
  // Filter States
  ...
  const [laporanCashbackData, setLaporanCashbackData] = useState<LaporanCashbackItem[]>([]);
  const [loading, setLoading] = useState(false);

  const fetchLaporanCashback = async () => {
    setLoading(true);
    try {
      // Data mockup diset kosong sesuai tampilan referensi (dapat diisi data uji jika diperlukan)
      setLaporanCashbackData([]);
    } catch (error) {
      console.error('Gagal mengambil data laporan cashback:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const totalJumlahBonusSum = laporanCashbackData.reduce((acc, curr) => acc + curr.jumlahBonus, 0);

  return (
    <div className="w-full px-2 sm:px-4 py-4 space-y-4">
      {/* Header & Breadcrumb */}
      <div className="px-2">
        <h1 className="text-2xl font-normal text-gray-800 dark:text-gray-100">Laporan Cashback</h1>
        <div className="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2">
          <Link href="/admin" className="text-blue-600 hover:underline">Dashboard</Link>
          <span>/</span>
          <span>Laporan Cashback</span>
        </div>
      </div>
      ...
      <div className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded shadow-sm overflow-hidden">
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <button 
              type="button"
              onClick={() => alert('Mengekspor data laporan cashback...')}
            ...
            <div className="overflow-x-auto w-full">
              <table className="w-full text-left border-collapse text-sm border border-gray-300 dark:border-gray-700">
                ...
                <tbody className="divide-y divide-gray-300 dark:divide-gray-700 text-gray-700 dark:text-gray-300">
                  {laporanCashbackData.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="py-8 text-center text-gray-500 dark:text-gray-400 text-sm italic">
                        Tidak ada data
                      </td>
                    </tr>
                  ) : (
                    laporanCashbackData.map((item, index) => (
          ...
          <div className="text-right text-xs text-gray-500 dark:text-gray-400 pt-2">
            Menampilkan sampai dari total {laporanCashbackData.length} baris
          </div>
        </div>
      </div>
    </div>
