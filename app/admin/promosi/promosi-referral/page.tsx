...
interface PromosiReferralItem {
  id: number;
  keterangan: string;
  kalkulasiBerdasarkan: string;
  permainan: string;
  tipeDurasi: string;
  nilai: string;
...
export default function PromosiReferralPage() {
  const [referralData, setReferralData] = useState<PromosiReferralItem[]>([]);
  const [loading, setLoading] = useState(false);

  const fetchPromosiReferral = async () => {
    setLoading(true);
    try {
      ...
      setReferralData([
        {
          id: 1,
          keterangan: 'BONUS REFERRAL 1%',
          kalkulasiBerdasarkan: '(TO Lose - TO Win) x %',
          permainan: 'Slot, Live Casino',
          tipeDurasi: 'Setiap hari',
          nilai: '0.05%',
          bonusMaksimal: '',
          status: 'Aktif',
          waktuDibuat: '14 March 2025',
        },
      ]);
    } catch (error) {
      console.error('Gagal mengambil data promosi referral:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full px-2 sm:px-4 py-4 space-y-4">
      {/* Header & Breadcrumb */}
      <div className="px-2">
        <h1 className="text-2xl font-normal text-gray-800 dark:text-gray-100">Promosi Referral</h1>
        <div className="text-sm text-gray-500 dark:text-gray-400 mt-1 flex items-center gap-2">
          <Link href="/admin" className="text-blue-600 hover:underline">Dashboard</Link>
          <span>/</span>
          <span>Promosi Referral</span>
        </div>
      </div>
        ...
        <div className="p-4 space-y-4">
          {loading ? (
            <div className="py-12 text-center text-gray-400 italic text-sm">
              Memuat data promosi referral...
            </div>
          ) : referralData.length === 0 ? (
            <div className="py-12 text-center text-gray-400 italic text-sm">
              Tidak ada data promosi referral.
            </div>
          ) : (
              ...
              <table className="w-full text-left border-collapse text-sm border border-gray-300 dark:border-gray-700">
                <thead>
                  <tr className="border-b border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300 font-semibold bg-gray-100/80 dark:bg-gray-800/60">
                    ...
                    <th className="py-2.5 px-3 border-r border-gray-300 dark:border-gray-700">Kalkulasi Berdasarkan</th>
                ...
                <tbody className="divide-y divide-gray-300 dark:divide-gray-700 text-gray-700 dark:text-gray-300">
                  {referralData.map((item, index) => (
                    <tr key={item.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/20 transition">
                      ...
                      <td className="py-2.5 px-3 border-r border-gray-300 dark:border-gray-700">{item.kalkulasiBerdasarkan}</td>
          ...
          {!loading && referralData.length > 0 && (
            <div className="text-right text-xs text-gray-500 dark:text-gray-400 pt-2">
              Menampilkan 1 sampai {referralData.length} dari total {referralData.length} baris
            </div>
          )}
        </div>
      </div>
