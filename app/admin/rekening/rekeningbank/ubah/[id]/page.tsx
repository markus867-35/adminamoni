...
export default function UbahRekeningPage() {
  ...
  useEffect(() => {
    if (!id) return;
    const fetchRekeningDetail = async () => {
      setFetching(true);
      ...
      if (error) {
        ...
        setMemberGroup(data.member_group || 'Member Baru');
        setBankName(data.bank_name || '');
        setAccountName(data.account_name || '');
        setAccountNumber(data.account_number || '');
        setTipePotongan(data.tipe_potongan || 'Persen');
        setPotonganAdmin(data.potongan_admin !== null ? String(data.potongan_admin) : '0');
        setSembunyikan(data.sembunyikan || '0');
        setUrutan(data.urutan !== null ? String(data.urutan) : '0');
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Swal.fire({
        icon: 'success',
        title: 'Berhasil!',
        text: 'Data rekening bank berhasil diperbarui!',
        timer: 1500,
  ...
  return (
    <div className="w-full px-2 sm:px-4 py-4 space-y-4">
      ...
      <div className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded shadow-sm overflow-hidden">
        ...
        <form onSubmit={handleUpdate} className="p-6 space-y-4 max-w-4xl">
