...
const categories = ['SPORTSBOOK', 'VIRTUAL SPORTS', 'ESPORTS'];

export default function ManagementSports() {
  ...
  const fetchGames = async () => {
    try {
      ...
      console.error('Gagal memuat data sports:', error.message);
    } finally {
      setLoading(false);
    }
  };

  const filteredGames = games.filter((game) => (game.category || 'SPORTSBOOK').toUpperCase() === activeCategory);
    ...
    try {
      if (editingId) {
        const { error } = await supabase
          ...
          .eq('id', editingId);
        if (error) throw error;
        alert('Provider Sports berhasil diperbarui!');
      } else {
        const { error } = await supabase
          .from('sports_games')
          .insert([{ name, category: activeCategory, image_url: imageUrl, play_url: playUrl }]);
        if (error) throw error;
        alert('Provider Sports baru berhasil ditambahkan!');
      }
  ...
  const handleDelete = async (id, e) => {
    e.stopPropagation();
    if (!confirm('Yakin ingin menghapus provider sports ini?')) return;
    try {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            Total: <span className="font-bold text-yellow-400">{games.length}</span> Assets Sports terdaftar
          </p>
        </div>
              ...
              {loading ? (
                ...
                filteredGames.map((game, index) => (
                  <tr key={game.id} className={`transition ${darkMode ? 'hover:bg-blue-500/5' : 'hover:bg-gray-50'}`}>
                    ...
                    <td className="p-4">
                      <div 
                        onClick={() => router.push(`/admin/support/${game.name.toLowerCase()}?category=${activeCategory.toLowerCase()}`)}
                        ...
                        title="Klik untuk kelola support"
                      >
          ...
          <div className={`w-full max-w-lg p-6 sm:p-8 rounded-3xl border shadow-2xl relative ${darkMode ? 'bg-[#0b0e2d] border-blue-900 text-white' : 'bg-white border-gray-200 text-gray-900'}`}>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
              <div>
                ...
                <input
                  type="text"
                  placeholder="Contoh: SABA Sports / CMD368"
                  value={name}
